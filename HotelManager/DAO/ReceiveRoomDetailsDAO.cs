...
namespace HotelManager.DAO
{
    public class ReceiveRoomDetailsDAO
    {
        ...
        {
            string query = "CheckList @idReceiveRoom ";
            int count = DataProvider.Instance.ExecuteNoneQuery(query, new object[] { idReceiveRoom});
            return count > 0;
        }
        internal bool CheckLimit(int idReceiveRoom)
        {
            string query1 = "USP_LimitCustomer";
            int LimitCustomer = (int)DataProvider.Instance.ExecuteScalar(query1);
            string query2 = "USP_CheckLimit @idReceiveRoom";
            return DataProvider.Instance.ExecuteQuery(query2, new object[] { idReceiveRoom }).Rows.Count < LimitCustomer;
        }
    }
}
